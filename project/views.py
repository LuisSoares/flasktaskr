...
app=Flask(__name__)
#app.config.from_object fetches all variables in uppercase
...
def tasks():
    ...
    open_tasks=[dict(name=row[0],due_date=row[1],priority=row[2],task_id=row[3]) for
    row in cur.fetchall()]
    cur=g.db.execute("select name,due_date,priority,task_id from tasks where status=0")
    closed_tasks=[dict(name=row[0],due_date=row[1],priority=row[2],task_id=row[3]) for
    row in cur.fetchall()]
    g.db.close()
...
def new_task():
    ...
    if not name or not date or not priority:
        ...
        g.db.commit()
        g.db.close()
        flash('New entry was successfullt posted')
        return redirect(url_for('tasks'))
